...
uniform float uAlphaMode;   // 0 = cutout (alpha-test), 1 = blend (прозрачность), см. doc/render.md

void effect() {
    vec4 col = Texel(MainTex, VaryingTexCoord.xy);
    if (uAlphaMode < 0.5 && col.a < 0.5) discard;   // cutout: альфа-тест для листвы/травы
    ...
    float a = (uAlphaMode < 0.5) ? 1.0 : col.a;     // blend-пасс уважает альфу
